    ...
    : TargetProperty(name), IEquatable<Zone>
{
    ...
    /// </summary>
    public double Distance { get; set => field = CheckNonNegative(value, nameof(Distance)); } = CheckNonNegative(distance, nameof(distance));

    /// <summary>
    /// The zone size for the tool reorientation in radians.
    /// </summary>
    public double Rotation { get; set => field = CheckNonNegative(value, nameof(Rotation)); } = CheckNonNegative(rotation ?? (distance / 10).ToRadians(), nameof(rotation));

    /// <summary>
    /// The zone size for revolute external axis in radians.
    /// </summary>
    public double RotationExternal { get; set => field = CheckNonNegative(value, nameof(RotationExternal)); } = CheckNonNegative(rotationExternal ?? rotation ?? (distance / 10).ToRadians(), nameof(rotationExternal));
