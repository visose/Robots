...
public sealed class Val3CommandFormatter : CommandFormatter
{
    public static Val3CommandFormatter Instance { get; } = new();

    Val3CommandFormatter() { }

    protected override string? FormatDeclaration(Command command, RobotSystem system) =>
        command switch
        {
            SetAO value => VAL3Syntax.NumData(value.Name.NotNull(), value.Value),
            Wait value => VAL3Syntax.NumData(value.Name.NotNull(), value.Seconds),
            _ => null
        };
    ...
    protected override string? FormatCommand(Command command, RobotSystem system, Target target) =>
        command switch
        {
            Message value => $"putln({CommandText.Quote(value.Text)})",
            SetDO value => $"waitEndMove()\ndos[{value.DO}] = {CommandText.Boolean(value.Value, "true", "false")}",
            SetAO value => $"aioSet(aos[{value.AO}], {value.Name})",
            WaitDI value => $"waitEndMove()\nwait(dis[{value.DI}] == {CommandText.Boolean(value.Value, "true", "false")})",
            Wait value => $"waitEndMove()\ndelay({value.Name})",
            _ => null
        };
}

public static class Val3ProgramFile
{
...
{
    public CommandFormatter Commands => Val3CommandFormatter.Instance;

    public void Save(IProgram program, string folder) => Val3ProgramFile.Save(program, folder);

    public List<List<List<string>>> GetCode(RobotSystem system, Program program)
    {
        if (system is not SystemStaubli staubli)
            throw new ArgumentException("The VAL3 post processor requires a Staubli robot system.", nameof(system));

        PostInstance instance = new(staubli, program);
