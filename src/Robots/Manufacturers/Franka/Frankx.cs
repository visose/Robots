...
class FrankxPostProcessor : IPostProcessor
{
    ...
    public List<List<List<string>>> GetCode(RobotSystem system, Program program)
    {
        if (system is not SystemFranka franka)
            throw new ArgumentException("The Frankx post processor requires a Franka Emika robot system.", nameof(system));
    ...
    public void Validate(Program program, IReadOnlyList<ProgramTarget> targets)
    {
        if (program.RobotSystem is not SystemFranka system)
            throw new ArgumentException("The Frankx post processor requires a Franka Emika robot system.", nameof(program));
    ...
    class PostInstance
    {
        ...
        static double GetAccel(SystemTarget systemTarget)
        {
            ...
            return Clamp(speed.AxisAccel / (4 * PI), 0, 1);
        }
